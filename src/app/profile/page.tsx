...
export default function ProfilePage() {
  ...
  // استراتيجية البحث عن التذاكر: 
  // 1. إذا كان المستخدم مسجل ببريد، نبحث في كل الحجوزات (collectionGroup) التي تحمل بريده.
  // 2. إذا كان مجهول، نبحث في بروفايله الخاص أو الحجوزات التي تحمل UID الخاص به.
  const bookingsQuery = useMemoFirebase(() => {
    if (!firestore || !user) return null;
    
    // إذا كان لدى المستخدم بريد إلكتروني (مسجل دخول دائم)
    if (user.email) {
      return query(
    ...
    // إذا كان مجهول (Anonymous)، نبحث في مساره الخاص فقط
  ...
  const downloadTicket = async (bookingId: string) => {
    ...
    try {
      ...
      toast({ variant: "destructive", title: "خطأ في التحميل", description: "تعذر تحويل التذكرة لصورة." });
    } finally {
  ...
  if (isLoading) return (
    <div className="flex flex-col items-center justify-center p-20 gap-4">
      ...
      <p className="text-sm text-muted-foreground">جاري التحقق من هويتك...</p>
    </div>
  );
      ...
      {(!user || user.isAnonymous) && !bookings?.length ? (
        <div className="space-y-6 max-w-sm mx-auto pt-4">
          <header className="text-center space-y-2">
            ...
            <p className="text-xs text-muted-foreground">سجل دخولك لمشاهدة جميع تذاكرك السابقة</p>
          </header>
        ...
        <div className="space-y-8">
            ...
            <div className="flex items-center gap-5 p-6 bg-white rounded-[2rem] shadow-sm border border-primary/5">
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center gap-2 mt-1 text-muted-foreground">
                  ...
                  <p className="text-sm font-medium">{user?.email || profile?.email || "حساب ضيف"}</p>
                </div>
                <div className="flex gap-2 mt-2">
                  <Badge variant="secondary" className="bg-accent/10 text-accent border-none font-black px-4 py-1">
                    {isAdmin ? "المدير العام" : isStaff ? "موظف معتمد" : isDriver ? "قائد حافلة" : "عضو مسجل"}
                  </Badge>
                  {user?.isAnonymous && (
                    <Badge variant="outline" className="border-amber-200 text-amber-600 bg-amber-50">جلسة مؤقتة</Badge>
                  )}
                </div>
              </div>
            </div>
              ...
              <Card className="border-none bg-primary text-primary-foreground shadow-2xl rounded-[2.5rem] overflow-hidden">
                <CardContent className="p-7 flex items-center justify-between">
                  <div className="flex items-center gap-5">
                    ...
                    <div>
                      <h2 className="text-xl font-black">{isAdmin ? "المدير العام" : "قسم الموظفين"}</h2>
                      <p className="text-xs text-white/70">بوابة التحكم الإداري</p>
                    </div>
                  </div>
                  <Button asChild className="rounded-2xl bg-white text-primary hover:bg-white/90 font-bold px-6 h-12 shadow-xl">
          ...
          <section className="space-y-5">
            ...
            {isBookingsLoading ? (
                    ...
                    >
                      ...
                      <div className="p-8 space-y-8">
                         ...
                         <div className="flex items-center justify-between pt-6 border-t border-dashed border-primary/20">
                            <div className="text-right">
                              <p className="text-[10px] text-muted-foreground font-black mb-1 flex items-center gap-1 justify-end"><Navigation className="h-2 w-2" /> رقم تتبع الحافلة</p>
                              <p className="font-black text-lg text-primary font-mono">{booking.busTripId}</p>
                              <p className="text-[10px] text-muted-foreground mt-1">{new Date(booking.bookingDate).toLocaleDateString('ar-EG', { dateStyle: 'long' })}</p>
                            </div>
                            <div className="text-left">
                               <p className="text-[10px] text-muted-foreground font-black mb-1">رقم الحجز</p>
                               <p className="font-mono text-xs opacity-50">{booking.trackingNumber || booking.id.slice(0, 8)}</p>
                               <div className="h-12 w-12 mt-1 bg-muted/30 rounded-xl flex items-center justify-center">
              ...
              <div className="text-center p-20 bg-muted/10 rounded-[2.5rem] border-2 border-dashed">
                ...
                <p className="text-sm text-muted-foreground">لا توجد حجوزات مرتبطة بهذا الحساب حالياً</p>
              </div>
            )}
          </section>
          ...
          {user.isAnonymous && bookings?.length > 0 && (
            <div className="p-6 bg-primary/5 rounded-[2rem] border border-primary/10 text-center space-y-3">
              <p className="text-xs font-bold text-primary">هل تريد حفظ تذاكرك للأبد؟</p>
              <p className="text-[10px] text-muted-foreground">قم بإنشاء حساب بكلمة مرور باستخدام نفس البريد الإلكتروني الذي استخدمته في الحجز لتتمكن من الوصول لتذاكرك من أي جهاز.</p>
              <Button variant="outline" onClick={() => { handleLogout(); setAuthMode('register'); }} className="h-10 rounded-xl px-6 text-xs font-bold border-primary text-primary">إنشاء حساب دائم</Button>
            </div>
          )}
        </div>
      )}
